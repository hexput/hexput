...
**Next step:** Story 2.8, tracing every request back to its Client ID — a structured `tracing` span field on every event handled for a request, "no Client ID" before init, and JSON log output as a System Config option. Keep this section honest as stories land.
