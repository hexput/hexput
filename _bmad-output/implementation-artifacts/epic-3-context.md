...
- **CPU time and memory (Story 3.5).** `hexput_shared::budget::Dimension` is the six-dimension enum. `hexput-enforce::Budget` owns limits (1 s run time, 64 MiB) and the `budget.cpu_time_exceeded` / `budget.memory_exceeded` errors; the interpreter only meters (slices, an approximate live-bytes count with a ceiling); `hexput-exec` charges measured Script run time between slices, never Backend waits. "CPU time" is run time measured on the thread (LANGUAGE-REFERENCE §7).
- **No ambient names (Story 3.4).** `hexput_interpreter::BUILTINS` is the one list of builtin names and is empty; a fresh execution binds only its starting variables and top-level functions. Reading an undeclared name is `reference.undeclared_identifier`; only calling one is `capability.unknown_function`.
