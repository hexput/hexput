...
4. *Scope* — plain Registered Function calls only. Methods (`registerMethod(objKey, fn)`), the hidden `__secret` object metadata and per-entity reference ids are a separate story (see `deferred-work.md`).
