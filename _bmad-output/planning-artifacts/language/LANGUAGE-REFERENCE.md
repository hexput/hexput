...
- **Allocations** — every string, array or object the Script constructs: each evaluation of a string literal, each concatenation, each to-string conversion of a concatenation's non-string operand (`"a" + 1` is three: the literal, the conversion and the concatenation), each key a `for … in` hands out, and each array or object literal; plus one per *growth* of a collection, defined by the language and not by any implementation's storage policy: every append — an array element written at its length, or a new object key — that takes the collection's length past a power of two (from 1 to 2, 2 to 3, 4 to 5, 8 to 9, …). Scalars, rebindings, copies of a value, starting variables and values a host call returns do not count.
- **RPC calls** — every host call the Script makes, counted once its arguments are checked and before any capability decision: a refused, denied or failed call counts. A call whose arguments cannot be sent (a function or cyclic value, one nested past the argument depth, or too large for a frame) ends the Script with that argument's error and is not counted. A per-call handler's `Authorize` question is part of its call, not a second count.
- **Side effects** — every host call (counted exactly as RPC calls), plus every committed Global Variable write.
...
There is no import, require, module, filesystem, network, environment, or process facility in the grammar at all — the absence is structural, not a runtime check (Epic 3 Story 3.4). A call to an unregistered name raises `capability`, indistinguishable from a denied call. A Registered Function is callable only with a Capability grant: either a blanket grant the Backend gives it at registration, or — for a function registered without one — the Backend's per-call decision, asked anew on every call before anything runs on the Backend. Only an explicit `true` from that decision lets the call proceed; a refusal, an answer that is not a boolean, an error, no answer within the Daemon's authorization timeout, or a connection that ends first all deny it. Every denial is that same `capability.unknown_function` error, spanned on the call, with no `Call` sent — a Script cannot tell an unregistered name from a denied call, nor one reason for a denial from another — and, like every error, it ends the Script: there is no way to catch it. *(Stories 3.2, 3.3.)*
...
**[DECISION]** There is no built-in standard library at all in v2 — not even `len()` or `push()`. Everything a script can do beyond the operators above comes from Registered Functions. This keeps the trust boundary exactly at the capability edge (NFR1) and is the single most likely item to need revisiting once real scripts are written. **[2026-09-24, Story 3.4]** The builtin set is enumerated in one place, `hexput_interpreter::BUILTINS`, which is empty, and a test asserts that a fresh execution's root scope binds only its starting variables and its top-level named functions. Reading any other undeclared name — `fs`, `process`, `globalThis`, … — is `reference.undeclared_identifier`; calling one is `capability.unknown_function` (§8), and only a call is a capability denial — Story 3.4's "capability-denied" for a bare reference is superseded by §7/§8.
