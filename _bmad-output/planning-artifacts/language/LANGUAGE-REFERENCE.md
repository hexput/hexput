...
- **RPC calls** — every host call the Script makes, counted when it reaches the call, before any capability decision: a refused, denied or failed call counts. A per-call handler's `Authorize` question is part of its call, not a second count.
